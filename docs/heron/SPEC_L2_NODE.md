...
**Status: implemented, fails four of five gates.** Code in [`architectures/heron/node.py`](../../architectures/heron/node.py)
and [`architectures/heron/cortex.py`](../../architectures/heron/cortex.py), gates in [`cge/nodes.py`](../../cge/nodes.py),
...
`leak` now enforces it and `tests/test_heron_node.py` pins the ordering. **The fix changed no
