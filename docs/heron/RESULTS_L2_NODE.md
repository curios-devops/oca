...
`tests/test_heron_node.py` pins the ordering so it cannot silently break again.
...
The whole reason the benchmark lives in `core/` and `cge/` rather than inside an
