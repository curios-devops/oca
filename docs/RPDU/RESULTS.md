...
The detector itself works: [a test](../../tests/test_protocol.py) plants two groups of
...
hysteresis, and [tests](../../tests/test_landscape.py) prove that — but on this task it earns
...
gradient with respect to frequency ([test](../../tests/test_voting.py) pins the residue and
...
toward discrete hypotheses. A [test](../../tests/test_voting.py) now pins the collapse so it
...
> (`python -m bench.run --gates maze --seeds 0 1 2 --sides 12 24`):
>
> | | side 12 | side 24 |
> |---|---|---|
...
copy but every frame it sees is **byte-identical** ([test](../../tests/test_pa.py)). That
