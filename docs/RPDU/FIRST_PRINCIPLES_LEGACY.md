...
This is the RPDU line: `legacy/v1` (gradient-flow mesh), `legacy/v2` (oscillator mesh),
`legacy/pa` (Predictive Assembly). It is **frozen**. Nothing in `legacy/` changes again.
...
beat it has not earned its complexity. [tests/test_legacy_frozen.py](../../tests/test_legacy_frozen.py)
