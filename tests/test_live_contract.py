...
def _is_frozen(mod: str) -> bool:
    return any(mod == f or mod.startswith(f + ".") for f in FROZEN)
...
def test_the_live_architecture_directory_exists():
    ...
    would vacuously pass if the live package were renamed or moved. This assertion stops that,
    and it is not hypothetical: it has now happened twice -- once freezing `dcn/`, and once
    again moving everything under `architectures/`.
    """
...
def test_live_architecture_never_imports_from_a_frozen_one():
    ...
    Wren, Swift and Heron are available as opponents. None of them is a base class. Carry the
    *idea* across and re-justify it in the specs; never the code.
    """
    offenders = []
...
def test_a_layer_must_declare_its_horizon():
    ...
    """
    from architectures.corvus import Floor, Layer
...
def test_layers_address_only_the_layer_below():
    ...
    from architectures.corvus import Floor, Layer, check_stack
...
def test_all_three_frozen_architectures_are_still_reachable():
    ...
    for key in ("wren", "swift", "heron", "mirror"):
