...
class Cortex:
    ...
    **Layer 1 is `jay-v5.2`, the second candidate.** The first -- feature-at-place binding --
    was built, gated and refuted in a session: its readout was more sensitive to pose than to
    identity. It is kept executable in `retired/` and its entry names what this one had to have
    that it lacked: an orientation, not only a position.
